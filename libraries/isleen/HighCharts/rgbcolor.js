function RGBColor(a){this.ok=false;
if(a.charAt(0)=="#"){a=a.substr(1,6);
}a=a.replace(/ /g,"");
a=a.toLowerCase();
...
for(var e in c){if(a==e){a=c[e];
}}var b=[{re:/^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/,example:["rgb(123, 234, 45)","rgb(255,234,245)"],process:function(i){return[parseInt(i[1]),parseInt(i[2]),parseInt(i[3])];
...
this.getHelpXML=function(){var l=new Array();
for(var n=0;
n<b.length;
n++){var k=b[n].example;
for(var m=0;
m<k.length;
m++){l[l.length]=k[m];
}}for(var s in c){l[l.length]=s;
}var o=document.createElement("ul");
o.setAttribute("id","rgbcolor-examples");
for(var n=0;
n<l.length;
n++){try{var p=document.createElement("li");
var r=new RGBColor(l[n]);
var i=document.createElement("div");
i.style.cssText="margin: 3px; "+"border: 1px solid black; "+"background:"+r.toHex()+"; "+"color:"+r.toHex();
i.appendChild(document.createTextNode("test"));
var j=document.createTextNode(" "+l[n]+" -> "+r.toRGB()+" -> "+r.toHex());
p.appendChild(i);
p.appendChild(j);
o.appendChild(p);
}catch(q){}}return o;
