...
{strip}
	...
	<td>
		<i class="fa fa-trash-o deleteRow cursorPointer" title="{vtranslate('LBL_DELETE',$MODULE)}"></i>&nbsp;
		<i class="fa fa-rocket" title="{vtranslate('LBL_DRAG',$MODULE)}"></i>
	...
	<td>
		...
		<div>
			<input type="text" id="{$productName}" name="{$productName}" value="{$data.$productName}" class="productName form-control {if $row_no neq 0} autoComplete {/if}" placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}" data-validation-engine="validate[required]" {if !empty($data.$productName)} disabled="disabled" {/if} style="float: left;width: 85%;"/>
			...
			{if $row_no eq 0}
				...
				&nbsp;<i class="fa fa-times-circle clearLineItem cursorPointer" title="{vtranslate('LBL_CLEAR',$MODULE)}" style="vertical-align:middle"></i>
			{else}
		...
		{else}
			<div><br><textarea id="{$comment}" name="{$comment}" class="lineItemCommentBox form-control">{$data.$comment}</textarea>
		{/if}
	</td>
	<td>
		<input id="{$qty}" name="{$qty}" type="text" class="qty smallInputBox form-control" data-validation-engine="validate[required,funcCall[Vtiger_GreaterThanZero_Validator_Js.invokeValidation]]" value="{if !empty($data.$qty)}{$data.$qty}{else}1{/if}"/>
	...
	<td>
		<div>
			<input id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$listPrice)}{$data.$listPrice}{else}0{/if}" type="text" data-validation-engine="validate[required,funcCall[Vtiger_PositiveNumber_Validator_Js.invokeValidation]]" class="listPrice smallInputBox form-control"  style="float: left;width: 85%;"/>
			&nbsp;
			...
			<table width="100%" border="0" cellpadding="5" cellspacing="0" class="table table-nobordered popupTable">
			   ...
			   <tr>
					...
					<td>
						...
						<input type="text" id="discount_percentage{$row_no}" name="discount_percentage{$row_no}" value="{$data.$discount_percent}" class="discount_percentage smallInputBox pull-right discountVal form-control {if empty($data.$checked_discount_percent)}hide{/if}" />
					</td>
			   </tr>
			   <tr>
					...
					<td>
						<input type="text" id="discount_amount{$row_no}" name="discount_amount{$row_no}" value="{$data.$discount_amount}" class="smallInputBox pull-right discount_amount discountVal form-control {if empty($data.$checked_discount_amount)}hide{/if}"/>
					</td>
			   </tr>
			</table>
		...
		<span class="taxDivContainer">
			...
			{foreach key=tax_row_no item=tax_data from=$data.taxes}
			   ...
			   <tr>
				<td>
					<input type="text" name="{$taxname}" id="{$taxname}" value="{$tax_data.percentage}" class="smallInputBox taxPercentage form-control" />&nbsp;%
				</td>
				<td><div class="textOverflowEllipsis">{$tax_data.taxlabel}</div></td>
				<td>
					<input type="text" name="{$popup_tax_rowname}" class="cursorPointer smallInputBox taxTotal form-control" value="{$tax_data.amount}" readonly />
				</td>
			   </tr>
			{/foreach}
			</table>
	...
	<td>
		<div id="productTotal{$row_no}" align="right" class="productTotal" style="margin-bottom:10px;">{if $data.$productTotal}{$data.$productTotal}{else}0.00{/if}</div>
		<div id="discountTotal{$row_no}" align="right" class="discountTotal" style="margin-bottom:2px;">{if $data.$discountTotal}{$data.$discountTotal}{else}0.00{/if}</div>
		<div id="totalAfterDiscount{$row_no}" align="right" class="totalAfterDiscount"  style="margin-bottom:5px;">{if $data.$totalAfterDiscount}{$data.$totalAfterDiscount}{else}0.00{/if}</div>
