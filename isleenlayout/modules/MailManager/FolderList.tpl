...
{if $FOLDERS}
	<div class="quickWidget">
		<div class="accordion-heading accordion-toggle quickWidgetHeader FloatWidth">			
			<h5 class="title widgetTextOverflowEllipsis" style="padding: 0 10px;">{vtranslate('LBL_Folders',$MODULE)}</h5>
		</div>
			...
			<div>
				<div class="row-fluid">
					<div class="FloatWidth">
						<ul class="nav nav-list MailFold">
							{foreach item=FOLDER from=$FOLDERS}
								<li>
									<a class="mm_folder" id='_mailfolder_{$FOLDER->name()}' href='#{$FOLDER->name()}' onclick="MailManager.clearSearchString(); MailManager.folder_open('{$FOLDER->name()}'); ">
										...
										<span>
											{$FOLDER->name()} 
											<span class="label label-danger pull-right">{$FOLDER->unreadCount()}
											</span>
										</span>
										{else}{$FOLDER->name()}
										{/if}
