...
Reports_Detail_Js("Reports_ChartDetail_Js",{
 ...
 registerEventForPinChartToDashboard:function(){var a=this;
        jQuery("button.pinToDashboard").on("click",function(f){var c=a.getRecordId();
        var d=jQuery('input[name="primary_module"]').val();
        var b="ChartReportWidget_"+d+"_"+c;
        var g={module:"Reports",action:"ChartActions",mode:"pinChartToDashboard",reportid:c,title:b,mod:d};
        AppConnector.request(g).then(function(e)
        {                 
            if(e.result == 'false'){
                var i={text:app.vtranslate("Already Chart Pinned To Dashboard","Reports")};
                Vtiger_Helper_Js.showPnotify(i);
            }else{var h=app.vtranslate("Chart Pinned To Dashboard","Reports");
        var i={text:h,type:"info"};
        Vtiger_Helper_Js.showPnotify(i);
        }});
        });
    },
	registerEventForModifyCondition : function() {
		jQuery('button[name=modify_condition]').on('click', function() {
			...
			var classValue = icon.attr('class');console.info(classValue);
			if(classValue == "fa fa-chevron-right") {
