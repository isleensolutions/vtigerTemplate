...
jQuery.Class('Reports_Widget_Js',{
...
Vtiger_Widget_Js('Vtiger_Pie_Widget_Js',{},{
    ...
    generateLinks: function() {
        var e = this.getContainer().find(".widgetData").val();
        var g = JSON.parse(e);
        var f = [];
        for (var h = 0; 
        h < g.length; 
...
Vtiger_Widget_Js('Vtiger_Barchat_Widget_Js',{},{
	...
	loadChart : function() { 
        ...
        var rv = [];
      for (var i = 0; i < data['chartData'][0].length; ++i){
               rv[i] = data['chartData'][0][i];   
      } console.info(rv);           
        if(data['check'] == 'on'){ 
	    $(function () {
        var chart = new Highcharts.Chart({
        chart: {
            renderTo: 'chartcontent',
            type: 'column',
            margin: 75,
            options3d: {
                enabled: true,
