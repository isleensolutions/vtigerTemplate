			...
			<table class="table table-condensed themeTableColor">
				...
				<tbody>
					...
					{foreach key=FIELD_NAME item=FIELD_DETAILS from=$MODEL->getEditableFields()}
						<tr>
							<td width="40%"><label class="muted pull-right marginRight10px">{vtranslate($FIELD_DETAILS['label'], $QUALIFIED_MODULE)}</label>
							</td>
							<td style="border-left: none;" class="row-fluid">
								{if $FIELD_DETAILS['fieldType'] == 'picklist'}
									<span class="">
										<select class="select2 row-fluid col-md-6 paddingLeftRight" name="{$FIELD_NAME}">
								...
								{else}
									<input type="text" name="{$FIELD_NAME}" class="form-control" style="float: left;
    width: 50%;" data-validation-engine="validate[required, funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" {if $FIELD_VALIDATION[$FIELD_NAME]} data-validator={Zend_Json::encode([$FIELD_VALIDATION[$FIELD_NAME]])} {/if} value="{$FIELD_DATA[$FIELD_NAME]}" />
