...
{strip}
    <div class="accordion">
        <span><i class="fa fa-info-circle alignMiddle"></i>&nbsp;{vtranslate('LBL_CONFIGURE_DEPENDENCY_INFO', $QUALIFIED_MODULE)}&nbsp;&nbsp;</span>
    ...
    <div class="row-fluid" style="text-align:center">
        <div class="col-md-2 paddingLeftRight">
            <table class="table table-inbox table-condensed themeTableColor">
                <thead>
                    ...
                    {foreach item=TARGET_VALUE from=$TARGET_PICKLIST_VALUES name=targetValuesLoop}
                        <tr>
                            {if $smarty.foreach.targetValuesLoop.index eq 0}
                                <td class="tableHeading" style="border-top: medium none;">
                                    {$RECORD_MODEL->getTargetFieldLabel()}</td>
                        </tr>
                                {else}
        ...
        <div class="col-md-10 marginLeftZero dependencyMapping paddingRight">
            <table class="table  table-condensed themeTableColor pickListDependencyTable table-inbox">
                    ...
                    <div class="control-group">
                        <div class="controls row-fluid">
                            <label class="checkbox">
                                <input type="checkbox" class="sourceValue {Vtiger_Util_Helper::toSafeHTML($SOURCE_VALUE)}" data-source-value="{Vtiger_Util_Helper::toSafeHTML($SOURCE_VALUE)}" value="{Vtiger_Util_Helper::toSafeHTML($SOURCE_VALUE)}" 
                                {if empty($MAPPED_VALUES) || in_array($SOURCE_VALUE, array_map('decode_html', $MAPPED_SOURCE_PICKLIST_VALUES))} checked {/if}/>
                                &nbsp;{vtranslate($SOURCE_VALUE, $SELECTED_MODULE)}
                            </label>
                        </div>
                    </div>
                </td>
...
<div class="row-fluid padding1per">
    <span class="btn-toolbar paddingTop20px FloatWidth">
        <button class="btn btn-success" type="submit" style="margin-right: 5px;"><strong>{vtranslate('LBL_SAVE', $QUALIFIED_MODULE)}</strong></button>
