...
-->*}
{strip}
<ul class="nav nav-tabs massEditTabs" style="margin-bottom: 10px;">
...
<div class="tab-content layoutContent padding20 themeTableColor overflowVisible FloatWidth ">
	<div class="tab-pane active" id="allValuesLayout">	
		<div class="row-fluid">
			<div class="col-md-5 marginLeftZero textOverflowEllipsis">
				<table id="pickListValuesTable" class="table table-bordered table-hover table-condensed table-striped" style="table-layout: fixed">
					<thead>
					...
					{foreach key=PICKLIST_KEY item=PICKLIST_VALUE from=$PICKLIST_VALUES}
						<tr class="pickListValue cursorPointer" data-key="{Vtiger_Util_Helper::toSafeHTML($PICKLIST_VALUE)}">
							<td class="textOverflowEllipsis"><i class="fa fa-rocket"></i>&nbsp;&nbsp;{vtranslate($PICKLIST_VALUE,$SELECTED_MODULE_NAME)}</td>
						</tr>
					{/foreach}
					</tbody>
				</table>
			...
			<div class="col-md-2 row-fluid">
				{if $SELECTED_PICKLIST_FIELDMODEL->isEditable()}
					{if $SELECTED_PICKLIST_FIELDMODEL->isRoleBased()}
						<button class="btn col-md-12 marginLeftZero" id="assignValue">{vtranslate('LBL_ASSIGN_VALUE',$QUALIFIED_MODULE)}</button><br><br>
					{/if}	
					<button class="btn col-md-12 marginLeftZero" id="addItem">{vtranslate('LBL_ADD_VALUE',$QUALIFIED_MODULE)}</button><br><br>
					<button class="btn col-md-12 marginLeftZero" id="renameItem">{vtranslate('LBL_RENAME_VALUE',$QUALIFIED_MODULE)}</button><br><br>
					<button class="btn btn-danger col-md-12 marginLeftZero"  id="deleteItem">{vtranslate('LBL_DELETE_VALUE',$QUALIFIED_MODULE)}</button><br><br>
				{/if}
				<button class="btn btn-success col-md-12 marginLeftZero" disabled=""  id="saveSequence">{vtranslate('LBL_SAVE_ORDER',$QUALIFIED_MODULE)}</button><br><br>
			</div>
			<div class="col-md-5">
				<br><br><br>
				<div><i class="fa fa-info-circle"></i>&nbsp;<span>{vtranslate('LBL_DRAG_ITEMS_TO_RESPOSITION',$QUALIFIED_MODULE)}</span></div>
	...
	{if $SELECTED_PICKLIST_FIELDMODEL->isRoleBased()}
		<div class="tab-pane FloatWidth" id="AssignedToRoleLayout">
			<div class="row-fluid FloatWidth m-bot15">
				<div class="col-md-2" style="margin-top: 5px">{vtranslate('LBL_ROLE_NAME',$QUALIFIED_MODULE)}</div>
				<div class="col-md-7">
