...
<div class="alphabetSorting FloatWidth">
	<ul class="FloatWidth pagination pagination-sm SettingUsers">		
		{foreach item=ALPHABET from=$ALPHABETS}
		<li class="alphabetSearch textAlignCenter cursorPointer {if $ALPHABET_VALUE eq $ALPHABET} highlightBackgroundColor {/if}">
			<a id="{$ALPHABET}" href="#">{$ALPHABET}</a>
		</li>
		{/foreach}	
	</ul>
</div>
<div class="listViewEntriesDiv FloatWidth" style='overflow-x:auto;'>
	...
	<table class="table listViewEntriesTable table-hover table-inbox">
		<thead>
							...
							{foreach item=IMAGE_INFO from=$IMAGE_DETAILS}
								<div class=''>
									{if !empty($IMAGE_INFO.path) && !empty({$IMAGE_INFO.orgname})}
										<img src="{$IMAGE_INFO.path}_{$IMAGE_INFO.orgname}" style="height: 50px;width: 100%;">
									{/if}
								</div>
							{/foreach}
							{if $IMAGE_DETAILS[0]['id'] eq null}
								<div class=''>
									<img src="{vimage_path('DefaultUserIcon.png')}" style="height: 50px;width: 100%;">
								</div>
							{/if}
						</div>
						<div class='col-md-10'>
					...
					<div class="pull-right actions">
						<span class="actionImages">
							{if $IS_MODULE_EDITABLE && $LISTVIEW_ENTRY->get('status') eq 'Active'}
								<a id="{$MODULE}_LISTVIEW_ROW_{$LISTVIEW_ENTRY->getId()}_EDIT" href='{$LISTVIEW_ENTRY->getEditViewUrl()}'><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="fa fa-pencil"></i></a>&nbsp;
							{/if}
							{if $IS_MODULE_DELETABLE && $LISTVIEW_ENTRY->getId() != $USER_MODEL->getId() && $LISTVIEW_ENTRY->get('status') eq 'Active'}
								<a id="{$MODULE}_LISTVIEW_ROW_{$LISTVIEW_ENTRY->getId()}_DELETE" class="deleteRecordButton"><i title="{vtranslate('LBL_DELETE', $MODULE)}" class="fa fa-trash-o"></i></a>
							{/if}
						</span>
					</div>
				{/if}
