...
<div class="row-fluid input-prepend input-append input-group">
	<span class="clearReferenceSelection cursorPointer input-group-addon">
		<i class='fa fa-times-circle' title="{vtranslate('LBL_CLEAR', $MODULE)}"></i>
	</span>
	<input id="{$FIELD_NAME}_display" name="{$FIELD_NAME}_display" type="text" class="form-control {if $smarty.request.view eq 'Edit'} col-md-7 {else} col-md-8 {/if} marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
	...
	<span class="relatedPopup cursorPointer input-group-addon">
		<i class="fa fa-search relatedPopup" title="{vtranslate('LBL_SELECT', $MODULE)}" ></i>
	</span>
	...
	<span class="cursorPointer createReferenceRecord input-group-addon">
		<i class='fa fa-plus' title="{vtranslate('LBL_CREATE', $MODULE)}"></i>
	</span>
	{/if}
