	...
	{foreach item=SIDEBARWIDGET key=index from=$QUICK_LINKS['SIDEBARWIDGET']}
		<div class="quickWidget"> 
			<div class="accordion-heading accordion-toggle quickWidgetHeader" data-target="#{$MODULE}_sideBar_{Vtiger_Util_Helper::replaceSpaceWithUnderScores($SIDEBARWIDGET->getLabel())}"
				...
				<span class="pull-left arrow_icon"><i class="fa fa-angle-down imageElement"></i>
              {*  <img class="imageElement" data-rightimage="{vimage_path('arrowDown.png')}" data-downimage="{vimage_path('arrowRight.png')}" src="{vimage_path('arrowRight.png')}" />*}</span>
				<h5 class="title widgetTextOverflowEllipsis" title="{vtranslate($SIDEBARWIDGET->getLabel(), $MODULE)}" style="float: left;">{vtranslate($SIDEBARWIDGET->getLabel(), $MODULE)}</h5>
