...
{assign var="displayId" value=$FIELD_MODEL->get('fieldvalue')}
<div class="row-fluid input-prepend input-append input-group">
	<span class=" clearReferenceSelection cursorPointer input-group-addon">
		<i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_clear" class='fa fa-times-circle' title="{vtranslate('LBL_CLEAR', $MODULE)}"></i>
	</span>
	<input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" type="text" class="form-control {if $smarty.request.view eq 'Edit'} col-md-7 {else} col-md-8 {/if}	marginLeftZero autoComplete" {if !empty($displayId)}readonly="true"{/if}
	...
	<span class="relatedPopup cursorPointer input-group-addon">
		<i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_select" class="fa fa-search relatedPopup" title="{vtranslate('LBL_SELECT', $MODULE)}" ></i>
	</span>
	...
	<span class="cursorPointer createReferenceRecord input-group-addon">
		<i id="{$MODULE}_editView_fieldName_{$FIELD_NAME}_create" class='fa fa-plus' title="{vtranslate('LBL_CREATE', $MODULE)}"></i>
	</span>
	{/if}
</div>
{/strip}