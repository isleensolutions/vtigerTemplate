...
{foreach item=tax key=count from=$TAXCLASS_DETAILS}
	...
	<td class="fieldLabel col-md-3">
		<label class="muted pull-right control-label">
	{/if}
	...
	<td class="fieldValue col-md-3">
		<div class="col-md-12">
			<input type="text" class="detailedViewTextBox form-control {if $show_value eq "hidden"} hide {else} show {/if}" name="{$tax.taxname}" value="{$tax.percentage}" data-validation-engine="validate[funcCall[Vtiger_PositiveNumber_Validator_Js.invokeValidation]]" />
		</div>
	</td>
	{assign var="tax_count" value=$tax_count+1}
	{if $COUNTER eq 2}
		</tr><tr class="FloatWidth">
