...
<div class="input-prepend input-group">
	...
	<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" class="input-medium form-control" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]" name="{$FIELD_MODEL->getFieldName()}"
...
{else if ($FIELD_MODEL->get('uitype') eq '72') && ($FIELD_MODEL->getName() eq 'unit_price')}
	<div class="input-prepend FloatWidth">
		<div class=" input-group">			
			<span class="input-group-addon">{$BASE_CURRENCY_SYMBOL}</span>		
			<input id="{$MODULE}-editview-fieldname-{$FIELD_NAME}" type="text" class="form-control unitPrice" name="{$FIELD_MODEL->getFieldName()}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
...
<div class="input-prepend">
	<div class="row-fluid">
		<span class="col-md-1"><span class="add-on row-fluid">{$USER_MODEL->get('currency_symbol')}</span></span>
		<span class="col-md-7"><input type="text" class="row-fluid" name="{$FIELD_MODEL->getFieldName()}" data-validation-engine="validate[{if $FIELD_MODEL->isMandatory() eq true} required,{/if}funcCall[Vtiger_Base_Validator_Js.invokeValidation]]"
