				...
				{foreach item=MODULE_FIELD_MODEL key=MODULE_NAME from=$CONVERT_LEAD_FIELDS}
					<div class="accordion-group convertLeadModules">
						<div class="accordion-heading">
							<div data-parent="#leadAccordion" data-toggle="collapse" class="accordion-toggle table-bordered moduleSelection panel-heading" href="#{$MODULE_NAME}_FieldInfo">
								<input id="{$MODULE_NAME}Module" class="convertLeadModuleSelection alignBottom" data-module="{vtranslate($MODULE_NAME,$MODULE_NAME)}" value="{$MODULE_NAME}" type="checkbox" {if $MODULE_NAME != 'Potentials'} {if $MODULE_NAME == 'Accounts' && $ACCOUNT_FIELD_MODEL && $ACCOUNT_FIELD_MODEL->isMandatory()} disabled="disabled" {/if} {if $MODULE_NAME == 'Contacts' && $CONTACT_FIELD_MODEL && $CONTACT_FIELD_MODEL->isMandatory()} disabled="disabled" {/if} checked="" {/if}/>
									...
									<span class="tools pull-right">
										<a href="javascript:;">
											<i class="iconArrow {if $CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Accounts"} icon-inverted fa fa-chevron-up {elseif !$CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Contacts"} icon-inverted fa fa-chevron-up {else} icon-inverted fa fa-chevron-down {/if}alignBottom"></i>
										</a>
									</span>
							</div>
						</div>
