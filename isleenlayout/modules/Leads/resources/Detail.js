...
Vtiger_Detail_Js("Leads_Detail_Js",{
		...
		if(errorElement.length != '0') {
			...
			app.showModalWindow(data,function(data){
				if(typeof callBackFunction == 'function'){
					jQuery("#leadAccordion").niceScroll({styler:"fb",cursorcolor:"#e8403f", cursorwidth: '6', cursorborderradius: '10px', background: '#404040', spacebarenabled:false, cursorborder: ''});
	...
	registerConvertLeadEvents : function() {
		...
		container.on('hidden', '.accordion-body', function(e){
			...
			currentTarget.closest('.convertLeadModules').find('.iconArrow').removeClass('fa fa-chevron-up').addClass('fa fa-chevron-down');
		}).on('shown', '.accordion-body', function(e){
			var currentTarget = jQuery(e.currentTarget);
			currentTarget.closest('.convertLeadModules').find('.iconArrow').removeClass('fa fa-chevron-down').addClass('fa fa-chevron-up');
		});
